...
df = pd.read_csv(r"F:\8- Dimension Reduction(PCA) unsup learning\Assignments\heart disease.csv")
df.describe()
df.info()

df1 = df.drop(["target"],axis=1) #droppin column
...
h_complete = AgglomerativeClustering(n_clusters = 2, linkage = 'complete', affinity = "euclidean").fit(df1_norm) 
h_complete.labels_
...
df2= df.drop(["target"],axis=1) #droppin column
...
pca_data.columns = "comp0", "comp1", "comp2","comp3", "comp4", "comp5","comp6", "comp7", "comp8","comp9", "comp10"
final = pd.concat([df.target, pca_data.iloc[:, 0:5]], axis = 1)
...
df4 = final.drop(["target"],axis=1) #droppin column
...
h_complete = AgglomerativeClustering(n_clusters = 2, linkage = 'complete', affinity = "euclidean").fit(df4_norm) 
h_complete.labels_
...
from sklearn.cluster import	KMeans
df5= final.drop(["target"],axis=1) #droppin column
...
ax = df1.plot(x='age', y='chol', kind='scatter',figsize=(12,8))
df1[['age', 'chol', 'clust']].apply(lambda x: ax.text(*x), axis=1) 
...
ax2 = df2.plot(x='age', y='chol', kind='scatter',figsize=(12,8))
df2[['age', 'chol', 'clust']].apply(lambda x: ax2.text(*x), axis=1) 
